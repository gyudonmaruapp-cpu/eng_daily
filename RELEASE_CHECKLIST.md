...
- [ ] **実機のホーム画面にウィジェットを追加して見た目を確認する** **[要ユーザー]**
  - 小・中・大それぞれ追加してみて、文字がはみ出ていないか・和訳が正しく出るか確認
  - レイアウト・文字サイズなど、見た結果に応じて調整する（**Claude可**、フィードバックをもらえれば `targets/widget/widgets.swift` を修正する）
...
- [x] シェア機能：X / LINE / その他 の各ボタンが実機で正しく起動するか確認（LINE/Xは対象アプリが端末に入っていないとブラウザにフォールバックする点も要確認）
  ...
  - シェアテキストにマーケティングURL（`https://gyudonmaruapp-cpu.github.io/eng_daily/`）を追記し、共有経由の流入導線を確保した
