...
- [x] development build がビルド成功（3サイズ対応版）
- [x] **実機のホーム画面にウィジェットを追加して見た目を確認する**
  - 小・中・大すべて追加確認済み。レイアウト崩れなし
  - 初回はiOS側がウィジェット拡張の対応サイズ情報を古いまま握っていて、3サイズとも小サイズの見た目になる現象が発生。アプリを完全アンインストール→再インストールで解消（development buildを繰り返し上書きインストールした際に起きやすいキャッシュ由来の症状）
...
- [x] ビルドしたdevelopment buildを実機にインストールし、`npx expo start --dev-client` でJS側をつないで開発・確認する（コード変更のたびにネイティブビルドし直さなくてよい）
  - トンネル接続時、WSL上の`/mnt/c/...`配下だとMetroのファイル監視が効かず変更が反映されないことがあった → 変更が反映されないときはMetroを再起動（`--clear`付きで再実行）すれば直る
- [x] 設定画面のタブバーがホームインジケーターと重なってタップしづらい問題を修正
- [ ] 全画面を一通り触る（ホーム/アーカイブ/お気に入り/設定/シェア）
- [x] 通知：設定画面で時刻を変更 → 実際にその時刻に通知が届くか確認（許可ダイアログが出るはず）
- [x] 名言カードの画像保存：シェアシート →「画像として保存」→ 写真アプリに保存されるか確認
- [x] シェア機能：X / LINE / その他 の各ボタンが実機で正しく起動するか確認（LINE/Xは対象アプリが端末に入っていないとブラウザにフォールバックする点も要確認）
  - Instagramへの共有ボタンは削除済み。`expo-sharing`経由のシェアシート表示止まりで「Instagramに投稿する」動作にはならず、実機で意図通り動かなかったため
  - シェアテキストにApp StoreのURL（`https://apps.apple.com/jp/app/id6799548051`）を追記し、共有経由の流入導線を確保した。公開されるまでは404になる点に注意
- [x] 広告バナーが表示されるか確認（テストID段階でも表示はされるはず）
- [x] ウィジェットの追加・表示（小・中・大）を確認。日付が変わったときの更新は未確認
- [x] フォントサイズ（S/M/L）切り替えが全画面に反映されるか確認
  - 名言本文にしか反映されていなかったバグを修正済み（和訳・MEMO・著者名も対象に追加）
...
- [x] development ビルドで実機テスト（セクション6）完了
